//////////////////////////////////////////////////////////////////////
// UiGif.cpp: implementation of the CUiGif class.
...
inline int CUiGif::TGIFControlExt::GetPackedValue(enum ControlExtValues Value)
{
...
inline int CUiGif::TGIFLSDescriptor::GetPackedValue(enum LSDPackedValues Value)
{
	int nRet = (int)m_cPacked;
...
inline int CUiGif::TGIFImageDescriptor::GetPackedValue(enum IDPackedValues Value)
{
	int nRet = (int)m_cPacked;
...
BEGIN_MESSAGE_MAP(CUiGif, CStatic)
	//{{AFX_MSG_MAP(CUiGif)
	ON_WM_DESTROY()
	ON_WM_PAINT()
	ON_WM_LBUTTONDOWN()
...
BOOL CUiGif::Load(HGLOBAL hGlobal, DWORD dwSize)
{
...
BOOL CUiGif::Draw()
{
	if (!m_bIsInitialized)
	{
		TRACE(_T("Call one of the CUiGif::Load() member functions before calling Draw()\n"));
		return FALSE;
	};
...
SIZE CUiGif::GetSize() const
{
	return m_PictureSize;
}

BOOL CUiGif::Load(LPCTSTR szFileName)
{
	ASSERT(szFileName);
...
BOOL CUiGif::Load(LPCTSTR szResourceName, LPCTSTR szResourceType)
{
...
BOOL CUiGif::SkipNextGraphicBlock()
{
...
UINT CUiGif::GetSubBlocksLen(UINT nStartingOffset) const
{
	UINT nRet = 0;
...
enum CUiGif::GIFBlockTypes CUiGif::GetNextBlock() const
{
...
int CUiGif::GetNextBlockLen() const
{
...
UINT WINAPI CUiGif::_ThreadAnimation(LPVOID pParam)
{
	ASSERT(pParam);
	CUiGif *pPic = reinterpret_cast<CUiGif *> (pParam);
...
HGLOBAL CUiGif::GetNextGraphicBlock(UINT *pBlockLen,
...
BOOL CUiGif::IsGIF() const
{
	return m_bIsGIF;
}

BOOL CUiGif::IsAnimatedGIF() const
{
	return (m_bIsGIF && (m_arrFrames.size() > 1));
}

BOOL CUiGif::IsPlaying() const
{
	return m_bIsPlaying;
}

int CUiGif::GetFrameCount() const
{
	if (!IsAnimatedGIF())
		return 0;
...
COLORREF CUiGif::GetBkColor() const
{
...
BOOL CUiGif::PrepareDC(int nWidth, int nHeight)
{
...
void CUiGif::OnDestroy()
{
	Stop(); 
	CStatic::OnDestroy();
}

void CUiGif::SetBkColor(COLORREF clr)
{
...
#ifdef GIF_TRACING
void CUiGif::WriteDataOnDisk(CString szFileName, HGLOBAL hData, DWORD dwSize)
{
	CFile file;
...
BOOL CUiGif::SetPaintRect(const RECT *lpRect)
{
	return CopyRect(&m_PaintRect, lpRect);
}

BOOL CUiGif::GetPaintRect(RECT *lpRect)
{
	return CopyRect(lpRect, &m_PaintRect);
}

void CUiGif::OnLButtonDown(UINT nFlags, CPoint point) 
{
	//if(CRect(0, 0, 234, 30).PtInRect(point))
	//{
	//	AfxMessageBox(_T("left-top"));
	//}
	//if(CRect(0, 30, 234, 60).PtInRect(point))
	//{
	//	AfxMessageBox(_T("left-bottom"));
	//}
	//if(CRect(234, 0, 468, 30).PtInRect(point))
	//{
	//	AfxMessageBox(_T("right-top"));
	//}
	//if(CRect(234, 30, 468, 60).PtInRect(point))
	//{
	//	AfxMessageBox(_T("right-bottom"));
	//}
	CWnd* pParent = GetParent();
	while(NULL != pParent)
	{
		if(pParent->SendMessage(WM_UI_GIF_CLICK, GetDlgCtrlID(), NULL)) break;
