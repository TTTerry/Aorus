...
DWORD CSocketApp::Set(int nPort, CString sIP, int nType)
{
	...
	char cIP[256];
	memset(cIP,'\0',256);
	WideCharToMultiByte(CP_ACP,0,sIP,sIP.GetLength(),cIP,256,NULL,NULL);
...
DWORD CSocketApp::Send(CString sMeg)
{
	...
	memset(szBuff,'\0',SOCKET_MEG_SIZE);
	WideCharToMultiByte(CP_ACP,0,sMeg,sMeg.GetLength(),szBuff,SOCKET_MEG_SIZE,NULL,NULL);

	//strcpy(szBuff, sMeg.GetBuffer());
	...
	if(SOCKET_ERROR == sendto(m_socket, (char*)szBuff, sizeof(szBuff), 0, (LPSOCKADDR)&m_sockaddr, sizeof(m_sockaddr)))
	{
		m_nLastError = WSAGetLastError();
...
DWORD CSocketApp::Recv(CString& sMeg)
{
	...
	if(SOCKET_ERROR == recvfrom(m_socket, (char*)szBuff, sizeof(szBuff), 0, (LPSOCKADDR)&m_sockaddr, (int*)&socklen))
	{
		m_nLastError = WSAGetLastError();
		return SOCKET_ERR_RECV;
	}
	sMeg.Format(_T("%s"), A2W(szBuff));
	if(m_socket != INVALID_SOCKET) m_sLastRecv =  inet_ntoa(m_sockaddr.sin_addr);
	else return SOCKET_ERR_RECV;
	return SOCKET_OK;
}
...
DWORD CSocketApp::Send(void* sMeg, int nLen)
{
	...
	if(SOCKET_ERROR == sendto(m_socket, (char*)sMeg, nLen, 0, (LPSOCKADDR)&m_sockaddr, sizeof(m_sockaddr)))
	{
		m_nLastError = WSAGetLastError();
		return SOCKET_ERR_SEND;
	}
...
DWORD CSocketApp::Recv(void* sMeg, int nLen)
{
	...
	USES_CONVERSION;
	int socklen = sizeof(m_sockaddr);
	if(SOCKET_ERROR == recvfrom(m_socket, (char*)sMeg, nLen, 0, (LPSOCKADDR)&m_sockaddr, (int*)&socklen))
	{
		m_nLastError = WSAGetLastError();
		return SOCKET_ERR_RECV;
	}
...
DWORD CSocketApp::SendFile(CString sPath)
{
	...
	memset(szPath,'\0',SOCKET_MEG_SIZE);
	//strcpy(szPath, sPath.Mid(sPath.ReverseFind('\\') + 1, sPath.GetLength() - sPath.ReverseFind('\\') - 1));
	CString cstr=sPath.Mid(sPath.ReverseFind('\\') + 1, sPath.GetLength() - sPath.ReverseFind('\\') - 1);
	WideCharToMultiByte(CP_ACP,0,cstr,cstr.GetLength(),szPath,SOCKET_MEG_SIZE,NULL,NULL);
	...
	while( i64FileSizeSend.QuadPart > 0 )
	{
		...
		while(true)
		{
			...
			{
				dwNumberToRead = (DWORD)i64FileSizeSend.QuadPart;
			}
...
DWORD CSocketApp::RecvFile(CString sPath, bool bRecv)
{
	...
	while(i64FileSizeSend.QuadPart > 0)
	{
		...
		while(true)
		{
			...
			{
				nSize = (int)i64FileSizeSend.QuadPart;
			}
...
const CString CSocketApp::GetLastRecvIP(void)
{
	if(m_socket != INVALID_SOCKET) return m_sLastRecv;
	return _T("0.0.0.0");
}
...
ADAPTER_LIST CSocketApp::GetMacInfo(void)
{
    ...
    if ((dwRetVal = GetAdaptersInfo(pIpAdapterInfo, &ulOutBufLen)) == NO_ERROR) {
        ...
        while (pAdapter) {
			...
			do 
			{
				//AdapterInfo.sIPs.push_back(pIpAddrString->IpAddress.String);
				CString cStr=_T("");
				cStr.Format(L"%s",pIpAddrString->IpAddress.String);
				AdapterInfo.sIPs.push_back(cStr);
